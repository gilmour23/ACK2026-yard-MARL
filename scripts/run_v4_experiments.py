...
def run(root:Path,quick:bool=False):
    ...
    for seed in train_seeds:
        d=root/f'marl_seed{seed}';cfg=ResourcePPOConfig(total_steps=steps,seed=seed,arrival_rate_per_hour=RATE);train_resource_marl(cfg,d,bc_m);learned.append(('marl',d/'resource_marl_final.pt',True,True,f'Proposed_MARL_s{seed}'))
        d=root/f'marl_noresource_seed{seed}';cfg=ResourcePPOConfig(total_steps=steps,seed=seed,arrival_rate_per_hour=RATE,include_resource_state=False);train_resource_marl(cfg,d,bc_m);learned.append(('marl',d/'resource_marl_final.pt',True,False,f'MARL_noResource_s{seed}'))
        d=root/f'marl_nopro_seed{seed}';cfg=ResourcePPOConfig(total_steps=steps,seed=seed,arrival_rate_per_hour=RATE,enable_proactive=False);train_resource_marl(cfg,d,bc_m);learned.append(('marl',d/'resource_marl_final.pt',False,True,f'MARL_noProactive_s{seed}'))
        d=root/f'central_seed{seed}';cfgs=SinglePPOConfig(total_steps=steps,seed=seed,arrival_rate_per_hour=RATE);train_single_ppo(cfgs,d,bc_s);learned.append(('single',d/'single_ppo_final.pt',True,True,f'Centralized_PPO_s{seed}'))
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--out',default='v4_run');ap.add_argument('--quick',action='store_true');a=ap.parse_args();run(Path(a.out),a.quick)
